...
三件分别通过内置图像生成工具制作，完整提示词保存在 `prompts.json`，原稿在 `sources/`。通过最近邻采样、20 色以内调色板、二值透明通道转换为真实 32×32 PNG；四周保留透明边距。游戏使用的资源仍位于 `src/main/resources/assets/dynasty/textures/item/`，路径和模型引用保持不变。
