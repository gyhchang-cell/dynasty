...
## 维度性能与攻击轨迹修复（2026-09-22）

帝国士兵修正刷怪类别、减少自然生成、限制局部数量；旧存档过量野兵在加载时限制并分批消失，召唤/命名/拴绳部队保留。青龙偃月刀的龙头朝下对齐法阵中心，后羿弓改为稳定小范围锁敌并修正箭头法阵方向。补旧建筑越界写入防护。需完全重启游戏，不用新建存档；旧野兵清理会随游戏保存生效，更新前应备份存档。详见 [修复证据与备份说明](../docs/content/patrol-trajectory-fix-2026-09-22.md)。
