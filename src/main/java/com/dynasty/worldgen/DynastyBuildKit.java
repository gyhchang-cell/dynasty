...
public final class DynastyBuildKit {
    ...
    public static void roof(WorldGenLevel level, int cx, int y, int cz, int hx, int hz,
        ...
        while (layer < 12) {
            ...
            if (x0 >= x1 || z0 >= z1) {                        // 屋脊 / ridge
                for (int x = x0; x <= x1; x++) {
                    for (int z = z0; z <= z1; z++) {
                        DynastyFeaturePlacement.setBlock(level, new BlockPos(x, yy, z), ridge, 2);
                    }
                }
                DynastyFeaturePlacement.setBlock(level, new BlockPos(x0, yy + 1, z0), ridge, 2);
                break;
            }
            for (int x = x0; x <= x1; x++) {
                for (int z = z0; z <= z1; z++) {
                    if (x == x0 || x == x1 || z == z0 || z == z1) {
                        ...
                        DynastyFeaturePlacement.setBlock(level, new BlockPos(x, yy, z), facing(stair,inward), 2);
                    }
                }
            }
            for (int[] corner : new int[][]{{x0, z0}, {x0, z1}, {x1, z0}, {x1, z1}}) {
                DynastyFeaturePlacement.setBlock(level, new BlockPos(corner[0], yy, corner[1]), ridge, 2);
                if(layer==0)DynastyFeaturePlacement.setBlock(level, new BlockPos(corner[0], yy - 1, corner[1]), lantern(), 2);
            }
            if (layer == 0) {
                for (int x = x0 + 2; x <= x1 - 2; x += 3) {
                    DynastyFeaturePlacement.setBlock(level, new BlockPos(x, yy - 1, z0 + 1), lantern(), 2);
                    DynastyFeaturePlacement.setBlock(level, new BlockPos(x, yy - 1, z1 - 1), lantern(), 2);
                }
                for (int z = z0 + 2; z <= z1 - 2; z += 3) {
                    DynastyFeaturePlacement.setBlock(level, new BlockPos(x0 + 1, yy - 1, z), lantern(), 2);
                    DynastyFeaturePlacement.setBlock(level, new BlockPos(x1 - 1, yy - 1, z), lantern(), 2);
                }
            }
            layer++;
        }
    ...
    public static void gableRoof(WorldGenLevel level, int x0, int y0, int z0, int x1, int z1,
        ...
        for (int layer = 0; layer <= half; layer++) {
            ...
            for (int x = x0 - 1; x <= x1 + 1; x++) {
                DynastyFeaturePlacement.setBlock(level, new BlockPos(x, yy, za), facing(stair,Direction.SOUTH), 2);
                if (zb != za) {
                    DynastyFeaturePlacement.setBlock(level, new BlockPos(x, yy, zb), facing(stair,Direction.NORTH), 2);
                }
            }
            if (za >= zb - 1) {
                for (int x = x0 - 1; x <= x1 + 1; x++) {
                    DynastyFeaturePlacement.setBlock(level, new BlockPos(x, yy + 1, (z0 + z1) / 2), ridge, 2);
                }
                break;
            }
        }
    ...
    public static void hallBody(WorldGenLevel level, int x0, int y0, int z0, int x1, int z1, int height,
        ...
        int mx = x0 + (x1 - x0) / 2;
        DynastyFeaturePlacement.setBlock(level, new BlockPos(mx, y0, z0), air(), 2);
        DynastyFeaturePlacement.setBlock(level, new BlockPos(mx, y0 + 1, z0), air(), 2);
        DynastyFeaturePlacement.setBlock(level, new BlockPos(mx + 1, y0, z0), air(), 2);
        DynastyFeaturePlacement.setBlock(level, new BlockPos(mx + 1, y0 + 1, z0), air(), 2);
        DynastyFeaturePlacement.setBlock(level, new BlockPos(mx, y0 + height, z0), plaque(), 2);
        for (int x = x0 + 2; x <= x1 - 2; x += 3) {
            DynastyFeaturePlacement.setBlock(level, new BlockPos(x, y0 + 1, z1), air(), 2);
        }
        for (int z = z0 + 2; z <= z1 - 2; z += 3) {
            DynastyFeaturePlacement.setBlock(level, new BlockPos(x0, y0 + 1, z), air(), 2);
            DynastyFeaturePlacement.setBlock(level, new BlockPos(x1, y0 + 1, z), air(), 2);
        }
    }
    ...
    public static void courtyard(WorldGenLevel level, int cx, int y, int cz, int halfX, int halfZ,
        ...
        for (int h = 0; h < 3; h++) {
            for (int x = x0; x <= x1; x++) {
                ...
                DynastyFeaturePlacement.setBlock(level, new BlockPos(x, y + h, z0), door ? air() : (h == 2 ? top : wall), 2);
                DynastyFeaturePlacement.setBlock(level, new BlockPos(x, y + h, z1), h == 2 ? top : wall, 2);
            }
            for (int z = z0; z <= z1; z++) {
                DynastyFeaturePlacement.setBlock(level, new BlockPos(x0, y + h, z), h == 2 ? top : wall, 2);
                DynastyFeaturePlacement.setBlock(level, new BlockPos(x1, y + h, z), h == 2 ? top : wall, 2);
            }
        }
        for (int side : new int[]{-2, 2}) {
            column(level, cx + side, y, y + 2, z0, post);
            DynastyFeaturePlacement.setBlock(level, new BlockPos(cx + side, y + 3, z0), lantern(), 2);
        }
    ...
    public static void materials(WorldGenLevel level, RandomSource rand, int x, int y, int z,
        ...
        for (int index = 0; index < size; index++) {
            ...
            DynastyFeaturePlacement.setBlock(level, new BlockPos(x + dx, y + rand.nextInt(2), z + dz), state, 2);
        }
    }
}
